...
const Option3 = () => {
  ...
  useEffect(() => {
    const fetchFunds = async () => {
      try {
        const response = await axios.get('http://localhost:5000/option3', {
          headers: {
  ...
  return (
    <div className="funds-container max-w-7xl mx-auto p-4">
      {selectedFund ? (
        ...
        <>
          <h1 className="text-4xl font-bold text-center mb-8">Medication Funds</h1>
          <ul className="funds-list grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {funds.length > 0 ? (
              ...
              <p className="text-center text-lg">No Medication funds found</p>
            )}
          </ul>
        </>
      )}
